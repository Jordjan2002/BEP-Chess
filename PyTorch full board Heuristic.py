...
from helpers import pos_to_representation, rep_to_move, move_to_rep, custom_reset, custom_step, custom_step_DQN, king_distance_reward_simple
    ...
    for episode in count():
        ...
        if obs.turn:
            ...
            if done:
                ...
                best_move_black, score = min(move_scores,  key = lambda x: x[1])
            ...
            if done: # black captures white piece
                if not random_move: 
                    piece_captured.append(1)
                # episode_durations.append(t + 1)
                # plot_durations()
                break
            
            if not random_move:
                piece_captured.append(0)
...
aggregate_pieces_lost = []
select_move_count = []
for i in range(0, len(piece_captured), 1000):
    average_reward = sum(piece_captured[-i:])/len(piece_captured[-i:])
    aggregate_pieces_lost.append(average_reward)
...
plt.figure(1)
plt.plot(select_move_count, aggregate_pieces_lost, label='Keeping piece rate')
plt.title('Rate of losing piece when picking greedy move')
plt.xlabel('# greedy select action')
plt.ylabel('Rate of piece losing move')
plt.ioff()
plt.show()
...
torch.save(policy_net.state_dict(), r"C:\Users\20203734\OneDrive - TU Eindhoven\Yearfour\BEPChess\model-heuristic.pth")
