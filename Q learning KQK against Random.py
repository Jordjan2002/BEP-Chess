...
import gym_chess
import random
from helpers import custom_reset, custom_step_reward, find_max_Q_move_KQK, pos_to_index_KQK, king_distance_reward_simple, custom_step
import numpy as np
import copy
import pandas as pd
...
Q_table_black = np.ones([squares, squares, squares,nr_of_pieces_black,squares])*50
...
for episode in tqdm(range(episodes+1)):
    ...
    while not done and len(played_moves) < 100: # with 50 move rule
        ...
        if obs.turn: # if White to move
            ...
            best_move_t1 = random.choice(env.legal_moves)
            ...
            best_move = random.choice(env.legal_moves)        
            obs, reward, done, info = env.step(env, best_move)
            played_moves.append(best_move)
...
#Q_table.tofile('Q-table.csv', sep = ',')