...
for chess_board in chess_boards:
    ...
    for episode in tqdm(range(episodes+1)):
        ...
        Fen = generate_endgame_FEN(chess_board, [chess.ROOK])
        obs = env.reset(env, Fen)
        visited_positions.append(obs.board_fen())
        done = False
        nr_moves = 0
        ...
        while not done and len(played_moves) < 50: # with 50 move rule
            ...
            if obs.turn: # if White to move
                ...
                moves = [move for move in moves if move.to_square in chess_board] # only regard moves in smaller board
                ...
                if np.random.random() > white_epsilon: # chance based on epsilon to look into random move
                    ...
                    Q_value_best_move, best_move, a, b = find_max_Q_move(moves, Q_table, obs, K, R, k, random_move=True)
                ...
                Q_value_best_move_t1, best_move_t1, a_t1, b_t1 = find_max_Q_move(moves, Q_table, obs, K_t1, R_t1, k_t1) # wel of geen t_1 erbij?????
                ...
                if done_t1: # if black can end the game
                    ...
                    Q_value_best_move_t2, best_move_t2, a_t2, b_t2 = find_max_Q_move(moves, Q_table, obs, K_t2, R_t2, k_t2)
                ...
                Q_value_best_move_t1, best_move_t1, a_t1, a_t2 = find_max_Q_move(moves, Q_table, obs, K_t1, R_t1, k_t1)
                obs_t1, reward_t1, done_t1, info_t1 = new_env.step(new_env, best_move_t1)
        ...
        if checkmate: #or reward == -1: # last two to make sure it prints it out well if checkmate with 4x4 board
                results.append(1)
...
# code for printing out the results
    ...
    print('positions visited:', len(set(visited_positions)), 'average time each position visited:', len(visited_positions)/len(set(visited_positions)))
    print('average win rate:', average_reward)
...
plt.title('Q-learning King Rook vs king on different board sizes')
plt.xlabel('episode')
plt.ylabel('value')
plt.legend()
plt.show()
