...
def optimize_model():
    ...
    # final_next_states = torch.cat([s for s in batch.next_state
    #                                                 if s is None])
    
    all_rewards = torch.cat([s for s in batch.reward])
    ...
    with torch.no_grad():
        ...
        #next_state_values[~non_final_mask] = all_rewards[~non_final_mask].float()
...
with open("edgames-KRK2_13_06.pickle", "wb") as f:
    pickle.dump(endgames, f)


for episode in tqdm(range(len(endgames))):
    Fen = endgames[episode]
    ...
    for move_count in count():
        ...
        if obs.turn:
            ...
            if done:
                ...
                new_state = pos_to_representation(obs_t1) 
    ...
    if len(legal_moves_count) > 1 and len(legal_moves_count) % 1000 == 0:
        print(episode, 'aggregate_select legal move:', sum(legal_moves_count[-1000:])/len(legal_moves_count[-1000:]))
...
for i in range(0, len(legal_moves_count), window_size):
    window = legal_moves_count[i:i+window_size]
    if len(window) == window_size:
        average_reward = sum(window) / window_size
...
for i in range(0, len(episode_durations), window_size):
    window = episode_durations[i:i+window_size]
    if len(window) == window_size:
        average_reward = sum(window) / window_size
...
with open(f"aggregate_episode_rewards_experiment_13_06.pickle", "wb") as f:
...
torch.save(policy_net.state_dict(), r"C:\Users\20203734\OneDrive - TU Eindhoven\Yearfour\BEPChess\model40k_13_06.pth")
